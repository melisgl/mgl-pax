...
(deftest test-apropos ()
  ...
  (test-read-apropos-locative-types)
...
(deftest test-read-apropos-locative-types ()
  (is (endp (mgl-pax::read-apropos-locative-types "")))
  (is (endp (mgl-pax::read-apropos-locative-types " ")))
  (signals (error :pred "Cannot parse locative type")
    (mgl-pax::read-apropos-locative-types "fun"))
  (signals (error)
    (mgl-pax::read-apropos-locative-types "|x"))
  (is (equal (mgl-pax::read-apropos-locative-types "function")
             '(function)))
  (is (equal (mgl-pax::read-apropos-locative-types "function pax:clhs")
             '(function clhs)))
  (is (equal (mgl-pax::read-apropos-locative-types
              "  method   pax:macro pax:reader ")
             '(method macro reader)))
  (is (equal (mgl-pax::read-apropos-locative-types ":all") '(:all)))
  (is (equal (mgl-pax::read-apropos-locative-types ":lisp") '(:lisp)))
  (is (equal (mgl-pax::read-apropos-locative-types ":pseudo") '(:pseudo))))
...
(deftest test-pax-apropos* ()
  ...
  (is (match-values (mgl-pax::pax-apropos*-process-args
                     ":|f o| function pax:macro"
                     "\"x y\"")
        (uninterned-symbol= * "f o")
        (equal * '(function macro))
        (equal * "x y")))
