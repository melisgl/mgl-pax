...
- Supports string names (called designators in Definitions). PAX needs
  to distuinguish those from symbol-based ones.
...
* MAYBE DREF-APROPOS: allow :LOCATIVE-TYPES match subclasses?
This may need to know that FUNCTION-DREF belongs to FUNCTION. Move
DEFCLASS FUNCTION-DREF into DEFINE-LOCATIVE-TYPE and
