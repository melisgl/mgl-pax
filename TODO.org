...
* MAYBE add TAB completion on the locative in the M-. prompt
This already works in C-.
...
* CANCELLED make C-. work on '"lambda list" pax:clhs'
It works on [lambda list][pax:clhs]. Actually, that would behave
differently from DOCUMENT.
* GO nested in GO?
* MAYBE "function. FOO" should not consider FUNCTION a locative
"BAR has a useful function, FOO doesn't."
* BUG (low) M-. vs DOCUMENT inconsistency
DOCUMENT-OBJECT* (method () (dref t)) works with M-.
DOCUMENT-OBJECT* `(method () (dref t))` works with DOCUMENT.
               ^
Acutally mgl-pax-find-locatives here finds the locative but not in Lisp mode.
...
* BUG mgl-pax-document FRAGMENT syntax is broken
...
* BUG pax::@PAX-URLs in DOCUMENT-PAX-URL-PATH are not parsed properly
It should invert DREF-TO-PAX-URL.

(defpackage "X Y")
(defun |x y| ()
  "sdaf"
  nil)
(urldecode (dref-to-pax-url (dref "X Y" 'package)))
(parse-url (urldecode (dref-to-pax-url (dref '|x y| 'function))))
