...
* MAYBE escape some html in docstrings?
This could be easily implemented as a parse tree transformation in
SANITIZE-DOCSTRING but:

1. An extra parse/print is costly (although we can avoid it if there
   are e.g. no < characters in the docstring at all).
2. 3bmd is not print/parse consistent enough for this to work well.
3. What if we do want to include html?

Instead, just make sure that the HTML does not mess things up by
closing open tags? That would still make unescaped tags disappear from
docstrings.

So, we need to be able to include html but in a way that's less likely
to be stumbled upon in docstrings.
