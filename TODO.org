...
Or maybe this is better thought of as a fallback to external link if
...
[ironclad][asdf:system] is a broken link if ironclad is not loaded
(live doc) or not being documented (offline).

Maybe a fallback to an URL (or nothing) would be useful.
...
package from non-link code.
** CANCELLED smart quotes
...
** TODO generate function, variable, etc indices
The scope and location must be  controllable by the caller; tacking on
indices after everything else in a PAX:DOCUMENT call is brittle.
** TODO make it possible to see ambiguous links live
When using the live browser to develop, it is useful to see how the
dead output will look like.
** MAYBE make *document-downcase-uppercase-code* default to T in sections?
What if @DOCUMENTABLE is a list of sections and random stuff?
** MAYBE make "a" and "an" locative aliases of the "type"
so that one can write "X is a STRING" without having to disambiguate
with "X is a [STRING][class]".
** MAYBE use https://codeberg.org/dlowe/metaspectre for something?
** TODO fix scalability of FIND-PARENT-SECTIONS
* web
...
* world
** document packages, systems like as in live browsing
* transcribe
...
** TODO consider font-locking codifiable and linkable stuff in docstrings
