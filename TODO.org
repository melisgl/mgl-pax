...
** BUG underscores in mathjax are escaped in pdf output
Probably not just underscores.

This comes about because the intermediate markdown generated is
escaped automatically (3BMD::PRINT-MD-ESCAPED), and it's hard to make
it miminal. The right solution seems to be to write a 3BMD mathjax
extension.
* web
