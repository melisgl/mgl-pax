...
But there is not good place to define them, so we'll something like a
symbol based definition:
...
the definition.
* BUG (low) C-. differs from M-. on "type FOO function"
- C-. goes to the disambiguation page of FOO, which lists all definitions
- M-. pops up only the TYPE and FUNCTION dspecs
- "type FOO function" links to (foo type) and (foo function)

One way to fix this would be to allow multiple locatives in pax urls
(parsing is implemened with :MULTIPLE-LOCATIVES-P in
READ-REFERENCE-FROM-STRING), but then pax-urls could link to 2 or 3
references while that is currently not possible from the documentation
(only 1 or all).
...
* TODO documentation browser through a web server
...
* BUG SBCL traced function location is wrong
